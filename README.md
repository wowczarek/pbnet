...
- **PBNET is slow.** On the PB-2000 side, everything is written in DL-Pascal + assembly. While this is the best performance you can get on this platform with a high-level language, it is still slow. With the core routines (block encoding, decoding and checksums) written in HD61700 assembly, processing is already not the limiting factor and the framing overhead is low (4 bytes per 190 bytes). Piotr Piatek's USB serial module is speed-limited on the RX side and maximum RX rate PBNET achieves is about 250 Bps + ~530 Bps TX. With the FTDI chip buffering, we don't have to worry about the speed with which we write to PB's port. For use with an FA-7 or MD-100 interface, PBNET supports rate-limiting (staggered transmit) (`-l` option, limit in Bps). Testing with a physical serial port is ongoing.
