...
- *PBNET is slow.* On the PB-2000 side, everything is written in DL-Pascal. While this is the best performance you can get on this platform with a high-level language, it is still slow. With the core routines (block encoding, decoding and checksums) were writen in HD61700 assembly, PBNET is able to achieve about 250 Bps (bytes per second) receive, and ~520 Bps transmit. Framing overhead is low (4 bytes per 190 bytes) and coding+decoding+checksums are already not the limiting factor. Sadly, even when just reading data without processing, the PB-2000 is unable to achieve more than ~250 Bps in, 530 Bps out. PBNET is being developed using Piotr Piatek's USB-serial module (http://www.pisi.com.pl/piotr433/usb100_e.htm), but further tests with FA-7 and MD-100 will verify the maximum throughput possible.

- *PBNET works with the help of a host computer, not with two PB-2000s directly.* Although by all means possible, other than for fun purposes, there isn't too much value in talking between two PB-2000s. You can still do it via the host - it is by all mean possible to have IP between two PBs, but this would mean flow control in the TX direction as well, which will make it slower. Maybe I will revisit this.
