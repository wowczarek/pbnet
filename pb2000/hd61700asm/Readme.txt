-------------------------------------------------------------------------
名称      HD61700 CROSS ASSEMBLER for Win32 (Rev 0.44)
動作環境  Win95/98/98SE/NT4/2000/XP/Vista/8/10
...
■ 機能
 PB-1000内蔵のアセンブラとほぼ同一(上位互換)ですが、以下の違いがあります。
  ...
  4.セカンドオペレーション拡張($0,$30,$31,LD&JR)等がデフォルトで指定可。
    (/n オプション指定にてOFF）
  5.出力形式は、BASICのDATA文形式とPBF形式(注1)またはPAS形式から選択できます。
...
■ オプション（省略可）
 アセンブル時に以下のオプションが指定できます。
  ...
  /s ---  PAS(DL-Pascal用inline)形式で出力する。
...
４．PAS形式
PB-2000C用のDl-Pascalカードで利用できる形式です。
Wojciech Owczarek氏が追加した形式でソースコードにinline()でマージして利用するとの事です。

/Sオプション付きでアセンブルすると、*.pas形式のファイルを出力します。
 >HD61 source.asm /s [enter]
 ->Output 'source.pas'.

*.pas 形式は、以下のようにinlineヘッダとデータが並んでいます。
-----
inline(
...
15.「DL-Pascal inline() assembler形式」Wojciech Owczarek氏 2021/7/8
...
追記 2021/7/9
何と10年ぶりの更新です。EU形式をサポートしたことで海外でも使ってくれている方がいたようで、
Wojciech Owczarekさんから機能追加と修正案を頂き、Rev0.44としてリリースしました。
手持ちのPB-1000もかなりくたびれてきていますが、この機会に少し弄ってやろうかと思っています。
...
Rev 0.44  2021.07.09  (1)/sオプションを追加。Dl-Pascal用のinline形式の出力をサポート。
                      (2)unsigned longをunsigned intに変更。
                       古いコンパイラ(DOS)用に使ってましたが最近のlinux系コンパイラとの相性が
                       よろしくないとの事で修正しました。
                      (3)ファイル最終を検知した際、2(EOFERR)をOS側(ERRORLEVEL)に戻していましたが
                       ファイル最終検知はエラーではないので正常(NORM)を戻すように修正。
                      (4)出力ファイルへのEOF付与を廃止しました。
                       DOSではEOF付与は便利な面もありましたが、最近のOSでは不要な場合も多く
                       廃止することにしました。
